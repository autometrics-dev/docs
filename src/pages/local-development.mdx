...
`am.toml` where you can define defaults that `am start` will use. This file uses
the [TOML](https://toml.io/) format.

### Example

The following example shows all the different configuration items. If used with
`am start`, it will start a pushgateway and adds two endpoints for Prometheus to
sample the metrics from:
