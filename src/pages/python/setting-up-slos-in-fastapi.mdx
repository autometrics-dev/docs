...
Our first step is to generate the necessary metrics for our SLOs. We can do this by decorating our path operation functions with `autometrics`.

```python filename="users.py" {1,5,10,15,20,25}
...
Using these utilities, we define our SLO which we can then pass to each of our `autometrics` decorated functions.
...
```python filename="main.py" {14,19,24,29,34}
from autometrics import autometrics
from autometrics.objectives import Objective, ObjectiveLatency, ObjectivePercentile
from fastapi import FastAPI
